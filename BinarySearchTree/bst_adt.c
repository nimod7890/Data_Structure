...
void screen() {
    ...
    printf("\n\t  A                          hide node data");//기능
    printf("\n\t  B                          reveal node data");//기능
    printf("\n\t  D                          degree of tree");//기능
    printf("\n\t  Dn                         degree of node");//기능
    printf("\n\t  H                          height of tree (start with 0)");//
    printf("\n\t  Hn                         level of node (start with 0)");//기능
    printf("\n\t  #                          count all node");//
...
int level_of_node(T* tree, NODE* cnode);
NODE* level_of_tree(T* tree, NODE* p1, NODE* p2);
        ...
        for (int i = 0; i < strlen(order); i++) {
            ...
            case '+'://
                if (data != 0) {
                    ...
                    if (tree->cnt == 0) {
                        ...
                        if (new != NULL) insert(tree, new, data);
                        else {
                            printf("\n\t중복 값은 입력할 수 없습니다.");
                            //중복 값 루트 출력
            ...
            case'H'://
                ...
                if (order[i + 1] == NULL) {
                    ...
                    if (new == NULL) {//값 존재하면
                        found = search(tree->root, data);//해당 값 반환//값 존재하기 때문에 NULL 반환하는 일 없음 
                        printf("\n\tlevel of node %d : %d", found->data, level_of_node(tree, found));
                    }
                    else printf("\n\tNot Found");//값 없으면
                }
                break;
...
int level_of_node(T* tree, NODE* cnode) {
    ...
    NODE* tmp = cnode;//해당 노드부터 시작
    NODE* pnode;//cnode의 get_parent 반환값으로 사용
    int cnt = 0;//get_parent 반복 횟수 

    while (tmp != NULL) {//부모노드 없으면 탐색 끝
        pnode = get_parent(tree->root, tmp->data);//노드의 상위 레벨 노드(부모 노드) 존재 여부 확인
        if (pnode != NULL) cnt++;//부모노드가 있으면 레벨 +1
        tmp = pnode;//부모노드 없을 경우 tmp==NULL ( 탐색 끝)
    }
    return cnt;//level 값 반환

}//getparent 피료
NODE* level_of_tree(T* tree, NODE* p1, NODE* p2) {//levelofnode 함수에 tree->root 필요하기 떄문에 T* tree 인수 사용
    
    if (p1 == NULL) return NULL;

    NODE* deeper = level_of_node(tree, p1) > level_of_node(tree, p2) ? p1 : p2;
    //p1, p2 레벨 비교해 더 높은 레벨인 노드를 deeper에 저장 

    deeper = level_of_tree(tree, p1->left_child, deeper) ? level_of_tree(tree, p1->left_child, deeper) : deeper;
    //p1의 leftchild와 new에 대해 leveloftree함수 적용
        //p1의 왼쪽이 NULL이면 deepter는 그대로 유지
        //p1의 왼쪽 값이 존재하면 levelofnode 계산해서 상위 레벨가진 노드 반환

    deeper = level_of_tree(tree, p1->right_child, deeper) ? level_of_tree(tree, p1->right_child, deeper) : deeper;
    //p1의 오른쪽에 대해서도 똑같이 적용

    return deeper;//level이 가장 큰 node반환
